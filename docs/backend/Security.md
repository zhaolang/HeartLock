...
| 匹配检测 | SHA-256(phone) | 确定性哈希，用于实时匹配检测（性能远优于 bcrypt 逐行验证） |
...
| 哈希类型 | 用途 | 特性 |
|---|---|---|
| bcrypt(cost=12) | 安全存储，防暴力破解 | 约 250ms 计算，不可逆 |
| SHA-256 | 匹配检测和查重 | 纳秒级计算，确定性输出，可建索引 |

- 手机号明文 -> 两种哈希均从用户输入计算
- bcrypt 用于安全场景（用户验证），SHA-256 用于性能场景（匹配查询）
- 两个哈希值均不反向推导原始手机号
...
**V1 简化实现说明：**
- 当前 V1 阶段使用主密钥（Master Key）直接加密心锁内容（AES-256-GCM）
- 无需单独管理数据密钥生命周期，降低了 KMS 复杂度
- **V2 升级方向**：引入数据密钥 + 主密钥双层架构

**V1 加密流程：**
1. 生成 12 字节随机 nonce
2. 使用主密钥（32 字节，HEX 编码）初始化 AES-256-GCM
3. 加密心锁明文内容 → ciphertext + nonce 存入 heart_locks 表
4. 解密时：用主密钥 + nonce 解密 ciphertext
...
**V2 数据密钥生命周期：**
1. 创建心锁时：生成随机 AES-256 数据密钥 → 用数据密钥加密内容 → 用主密钥加密数据密钥 → 存储加密后的数据密钥
...
V1 直接加密模式下，密钥轮换需要全量重加密所有非 DESTROYED 的心锁内容：

| 步骤 | 操作 | 说明 |
|---|---|---|
| 1 | 生成新主密钥 | 新密钥必须使用安全随机数生成器（crypto/rand），密钥强度 256 位 |
| 2 | 记录新旧密钥 | 在环境变量或配置中添加 NEW_MASTER_KEY，保留 OLD_MASTER_KEY |
| 3 | 渐进式重加密 | 后台任务读取所有活跃心锁 → 用旧密钥解密 → 用新密钥重新加密 → 更新 heart_locks 表 |
| 4 | 验证 | 随机抽样 100 条记录，确认新密钥可正常解密 |
| 5 | 移除旧密钥 | 重加密全部完成并验证通过后，移除 OLD_MASTER_KEY 配置 |

**V2 升级后轮换将更高效：**
- 仅需重新加密数据密钥（128 字节级别），无需全量重安心锁内容
- 可在线无缝完成轮换
